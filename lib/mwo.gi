...
#M  ArrowNC( <mwo> <kind>, <elt>, <tail>, <head> ) 
##
InstallMethod( ArrowNC, 
    "for mwo, kind, element, tail and head", true,  
    [ IsMagmaWithObjects, IsInt, IsMultiplicativeElement, IsObject, IsObject ],
    0, 
function( mwo, kind, e, t, h ) 
    ...
    if ( kind = 1 ) then 
        ...
        fam := IsMultiplicativeElementWithObjectsAndOnesFamily;
        elt := Objectify( IsMultiplicativeElementWithObjectsAndOnesType,
                   [ mwo, e, t, h ] );
    elif ( kind = 3 ) then
        fam := IsMultiplicativeElementWithObjectsAndAssociativeFamily;
        elt := Objectify( IsMultiplicativeElementWithObjectsAndAssociativeType,
                   [ mwo, e, t, h ] );
    elif ( kind = 4 ) then
        ...
        Error( "ArrowNC only defind for kind in [1,2,3,4]" );
    fi;
    return elt;
end );
...
function( mwo, e, t, h ) 
    ...
    if FamilyObj( piece ) = IsMonoidWithObjectsFamily then
        return ArrowNC( mwo, 2, e, t, h );
    elif FamilyObj( piece ) = IsSemigroupWithObjectsFamily then
        return ArrowNC( mwo, 3, e, t, h );
    else
        return ArrowNC( mwo, 4, e, t, h );
    fi;
end );
...
function( e1, e2 ) 
    ...
    if ( e1![4] = e2![3] ) then 
        return Arrow( e1![1], e1![2]*e2![2], e1![3], e2![4] );
    else 
        return fail;
    fi;
end );
...
function( e, p ) 
    ...
    if ( e![4] = e![3] ) then 
        return Arrow( e![1], e![2]^p, e![3], e![4] );
    else 
        return fail;
    fi;
end );
...
function( e, mwo )
    ...
    Info( InfoGroupoids, 3, "\in for a standard magma with objects" ); 
    if ( e![1] = mwo ) then
        return true;
    elif HasParentList( mwo ) then
        if ( e![1] in ParentList( mwo ) ) then
            return Arrow( mwo, e![2], e![3], e![4] ) <> fail;
        fi;
    else
        return IsSubdomainWithObjects( mwo, e![1] );
    fi;
end );
...
function( e, mwo )
    ...
    local p, obs, mag, rays, r3, r4, g;

    Info( InfoGroupoids, 3, "\in for a union of standard magmas with objects" ); 
    p := PieceOfObject( mwo, e![3] );
    if p = fail then 
        return false;
    else
        obs := ObjectList( p );
        if not ( e![4] in obs ) then
            return false;
        fi;
        mag := p!.magma;
        if ( "IsSinglePieceRaysRep" in RepresentationsOfObject(p) ) then
            mag := RootGroup( p );
            rays := RaysOfGroupoid( p );
            r3 := rays[ Position( obs, e![3] ) ];
            r4 := rays[ Position( obs, e![4] ) ];
            g := r3 * e![2] * r4^-1;
        else
            g := e![2];
        fi;
    ...
    for i in obs do 
        for j in obs do 
            for g in mgens do
                k := k+1;
                if ( kind = 3 ) then
                    gens[k] := ArrowNC( mwo, 3, g, i, j );
                else
                    gens[k] := ArrowNC( mwo, 4, g, i, j );
                fi;
            od;
        od;
    od;
...
function( mwo )
    ...
    id := One( m );
    gens1 := List( mgens, g -> ArrowNC( mwo, 2, g, o1, o1 ) );
    gens2 := ListWithIdenticalEntries( (nobs-1)^2, 0 );
    k := 0;
    for i in [2..nobs] do 
        gens2[k+1] := ArrowNC( mwo, 2, id, o1, obs[i] );
        gens2[k+2] := ArrowNC( mwo, 2, id, obs[i], o1 );
        k := k+2;
    od;
...
function( comps, dom )
    ...
    local kind, len, pieces, L, fam, filter, mwo, i, obs, plist, par;
...
function( D, U )
    ...
    local ok, genU, ngenU, i, a, hs, obj, p, compU;
    ...
    if not ( IsGroupoid( D ) and IsGroupoid( U ) ) then
        Error( "not yet implemented for general magmas with objects" );
    fi;
...
#M  ParentList 
## 
InstallMethod( ParentList, "method for a domain with objects", 
    true, [ IsDomainWithObjects ], 0,
function( dom ) 
    ...
    L := [ ];
    if HasParent( dom ) then
        p := Parent( dom );
        if HasParentList( p ) then
            L := Concatenation( ParentList( p ), [ p ] );
        else
            L := [ p ];
        fi;
    fi;
