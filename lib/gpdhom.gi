...
function ( hom ) 
    ...
    for i in [1..ngens] do 
        ...
        imgs[i] := ArrowNC( a![1], 1, img, imo, imo );
    od;
    return [ gens, imgs ];
end );
...
function ( map, e )
    ...
    return ArrowNC( rng, 1, g2, imo[pt1], imo[ph1] );
end );
...
function ( map, e )
    ...
    local src, rng, pe, pose, mape, part, re;

    Info( InfoGroupoids, 3, 
          "this is the fourth ImageElm method in gpdhom.gi" );
    src := Source( map );
    rng := Range( map );
    pe := PieceOfObject( src, e![3] );
    pose := Position( Pieces( src ), pe );
    if ( HasIsSinglePiece( rng ) and IsSinglePiece( rng ) ) then 
        mape := MappingToSinglePieceMaps( map )[pose];
    else 
        part := PartitionOfSource( map );
        if ( part = fail ) then 
            Error( "map does not have PartitionOfSource" );
        fi;
        pose := Position( part, [ pose ] );
        mape := PiecesOfMapping( map )[ pose ];
    fi;
    re := Arrow( pe, e![2], e![3], e![4] );
    if not ( re in Source( mape ) ) then 
        Error( "the element e is not in the source of mapping map" );
    fi;
    return ImageElm( mape, re );
end );
