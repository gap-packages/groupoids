...
function( m1, m2 )
    ...
    Info( InfoGroupoids, 3, "\* for magma mappings" );
    src1 := Source( m1 );
    rng1 := Range( m1 );
    src2 := Source( m2 );
    rng2 := Range( m2 );
...
function( map1, map2 )
    ...
    Info( InfoGroupoids, 3, "second method for \* with map2 a single piece" );
    src1 := Source( map1 );
    rng1 := Range( map1 );
    src2 := Source( map2 );
    rng2 := Range( map2 );
...
function( m1, m2 )
    ...
    Info( InfoGroupoids, 3, 
          "third method for \* with map1 & map2 to single pieces" );
    s1 := Source( m1 );
...
function( m1, m2 )
    ...
    Info( InfoGroupoids, 3, "method 2 for \* and maps from hom discrete gpds" );
    s1 := Source( m1 );
    r1 := Range( m1 );
    s2 := Source( m2 );
    r2 := Range( m2 );
...
function( m1, m2 )
    ...
    Info( InfoGroupoids, 3, "method 3 for \* with map1 from hom discrete" );
    s1 := Source( m1 );
    r1 := Range( m1 );
    s2 := Source( m2 );
    r2 := Range( m2 );
...
function ( map, e )
    ...
    local M1, C1, piece, pe, obs1, pom, pim, obs2, t2, h2, g2;

    Info( InfoGroupoids, 3, 
          "this is the second ImageElm function in mwohom.gi" );
    M1 := Source( map );
    if HasPieces( M1 ) then
        C1 := Pieces( M1 );
...
function ( map, elt )
    ...
    mag := Source( map );
    if not ( elt in mag ) then 
        Error( "the element elt is not in the source of mapping map" );
    fi;
    obs := mag!.objects;
    one := One( mag!.magma );
