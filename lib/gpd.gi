...
SUB_CONSTRUCTORS := Concatenation( 
    ...
    "1.  SubgroupoidByObjects[Op]( groupoid, list of objects );\n", 
    "2.  SubgroupoidBySubgroup[Op]( groupoid, group );\n", 
...
##  the Sub...Op versions doi not assign a parent
...
function( gpd ) 
    ...
    return List( [1..Length(obs)], 
                 i -> ArrowNC( gpd, 1, elts[i], root, obs[i] ) );
end );
...
function( gpd )
    ...
    if ( HasIsGroupoidByIsomorphisms( gpd ) 
        ...
        gens1 := List( mgens, g -> ArrowNC( gpd, 1, [ g, g ], o1, o1 ) );
    else 
        gens1 := List( mgens, g -> ArrowNC( gpd, 1, g, o1, o1 ) );
    fi;
    if ( HasIsDirectProductWithCompleteDigraph( gpd ) 
        ...
        rays := gpd!.rays;
        gens2 := List( [2..nobs], 
                       i -> ArrowNC( gpd, 1, rays[i], o1, obs[i] ) );
    fi;
...
function( gpd )
    ...
    local comp, c, i, gp, len, rgp, rays;
...
function ( G1, G2 )
    ...
    if not ( ( G1!.objects = G2!.objects ) and ( G1!.magma = G2!.magma ) ) then
        return false;
    fi;
    ...
    if ( HasIsGroupoidByIsomorphisms( G1 ) 
...
function( gpd, pair, o1, o2 ) 
    ...
    if not ( ImageElm( iso, g1 ) = g2 ) then 
        ...
        return ArrowNC( gpd, 1, pair, o1, o2 );
    fi;
end );

InstallOtherMethod( ArrowNC, 
    "for mwo, kind, pair of elements, tail and head", true,  
    [ IsGroupoidByIsomorphisms, IsInt, IsList, IsObject, IsObject ], 0,
function( gpd, kind, pair, t, h ) 
...
#############################################################################
## 
#M  ConjugateGroupoid( <gpd>, <sgpd>, <elt> )
## 
InstallMethod( ConjugateGroupoid, "<gpd>, <sgpd>, <elt>", true,
    [ IsGroupoid and IsSinglePiece, IsGroupoid, IsGroupoidElement ], 0, 
function( G, H, e )

    local genH, ims, conj;

    if not IsSubgroupoid( G, H ) then
        Error( "H should be a subgroupoid of G" );
    fi;
    if not ( e in G ) then
        Error( "e should be an element of G" );
    fi;
    genH := GeneratorsOfGroupoid( H );
    ims := List( genH, h -> h^e );
    conj := SinglePieceSubgroupoidByGenerators( G, ims );
    return conj;
end );

InstallMethod( ConjugateGroupoid, "<gpd>, <elt>", true,
    [ IsGroupoid and IsPiecesRep, IsGroupoid, IsGroupoidElement ], 0, 
function( U, H, elt )

    local p, q, np, nq, n, pieces, G;
    ...
    G := pieces[n];
    pieces[n] := ConjugateGroupoid( G, H, elt );
    ...
    return IteratorByFunctions( rec( 
        ...
        NextIterator := function( iter )
            ...
            if ( hc!.type = "h" ) then 
                ...
                return ArrowNC( gpd, 1,
                           (hc!.trays[iter!.tpos]![2]^-1)*(iter!.gpelt),  
                           iter!.tobs[iter!.tpos], iter!.hobs[iter!.hpos] );
            elif ( hc!.type = "l" ) then 
                return ArrowNC( gpd, 1, 
                           (iter!.gpelt)*(hc!.hrays[iter!.hpos]![2]), 
                           iter!.tobs[iter!.tpos], iter!.hobs[iter!.hpos] );
            elif ( hc!.type = "d" ) then 
                return ArrowNC( gpd, 1, 
                           (hc!.trays[iter!.tpos]![2]^-1)
                             * (iter!.gpelt) 
                               * hc!.hrays[iter!.hpos]![2],  
                           iter!.tobs[iter!.tpos], iter!.hobs[iter!.hpos] );
            fi;
        end,
        ShallowCopy := iter -> 
...
function( gpd )
    return IteratorByFunctions( rec( 
        ...
        NextIterator := function( iter )
            ...
            if ( HasIsDirectProductWithCompleteDigraph( gpd ) 
                 and IsDirectProductWithCompleteDigraph( gpd ) ) then 
                return ArrowNC( gpd, 1, iter!.gpelt, 
...
InstallGlobalFunction( Subgroupoid, function( arg )
    ...
    local nargs, gpd, id, rays, gp, sub, L;
...
############################################################################
##
#M  SubgroupoidBySubgroupOp
#M  SubgroupoidBySubgroup
##
InstallMethod( SubgroupoidBySubgroupOp, 
    "for single piece groupoid and subgroup", true,
    [ IsGroupoid and IsSinglePiece, IsGroup ], 0,
function( G, sgp ) 
...
function( G, sgp ) 
    ...
    U := SubgroupoidBySubgroupOp( G, sgp );
    SetParentAttr( U, G );
    L := ParentList( U );
    return U;
end );
...
#############################################################################
##
#M  SubgroupoidByObjectsOp
#M  SubgroupoidByObjects
##
InstallMethod( SubgroupoidByObjectsOp, "for direct product groupoid + objects", 
...
function( G, obsU )
    ...
    return SinglePieceGroupoidNC( G!.magma, obsU );
end );
...
function( G, obsU )
    ...
    U := SubgroupoidByObjectsOp( G, obsU );
    SetParentAttr( U, G );
    L := ParentList( U );
    return U;
end );

InstallMethod( SubgroupoidByObjectsOp, "for a connected groupoid + object set", 
    true, [ IsGroupoid and IsSinglePieceRaysRep, IsHomogeneousList ], 0,
function( G, obsU )

    local obsG, nobs, ro, gpU, pos, rayG, rayU, ray1, j;
    ...
    od;
    return SinglePieceGroupoidWithRays( gpU, obsU, rayU );
end );
...
function( G, obsU )
    ...
    local obsG, nobs, ro, gpU, pos, rayG, rayU, ray1, j, U, L;
...
InstallMethod( SubgroupoidByObjectsOp, "for a groupoid and set of objects", 
...
function( G, sobs ) 
    ...
    for j in [1..Length(pieceG)] do
        ...
        if ( sobsP <> [ ] ) then 
            Add( pieceU, SubgroupoidByObjectsOp( P, sobsP ) );
        fi;
    od;
...
function( G, sobs ) 
    ...
    Info( InfoGroupoids, 2, "calling SubgroupoidByObjects, method 3" );
    U := SubgroupoidByObjectsOp( G, sobs );
    SetParentAttr( U, G );
    L := ParentList( U );
    return U;
end );
...
function( pgpd, rgp, rays )
    ...
    local obs, rob, fam, filter, gpd, L, id;
...
function( G, gens ) 
    ...
    if not ForAll( gens, g -> g in G ) then 
        Error( "generators gens not all in G" );
    fi;
    ngens := Length( gens );
...
function( gpd, piecedata )
    ...
    local name, p1, withrays, pieceU, len, i, pi, par, sub, U, c, piece, L,
        ...
        for i in [1..len] do 
            ...
            if withrays then 
                if not ( Length( pi ) = 3 ) then 
                    ...
                    od;
                    par := SubgroupoidByObjectsOp( gpd, pi[2] );
                ...
                sub := SubgroupoidByObjectsOp( gpd, pi[2] );
                if ( Length( pi ) = 2 ) then 
                    sub := SubgroupoidBySubgroupOp( sub, pi[1] );
                else
...
function( G, gps, obs )
    ...
    local pieceU, U, len, o, pieceG, obsg, C, i, gpo, ishomo, L;
...
function( gpd )
    ...
    local obs, len, gps, i, piece, U, ok, L;
...
function( gpd )
    ...
    local pieces, subs, U, L;

    pieces := Pieces( gpd );
    subs := List( pieces, FullTrivialSubgroupoid );
    U := UnionOfPieces( subs );
    SetParentAttr( U, gpd );
    L := ParentList( U );
    return U;
end );
...
function( gpd, sgpd, e ) 
    ...
    local g, t, h, G, U, obsU, nobsU, post, arayU, rt, arayt, gpt, rc, rcos;
    ...
    obsU := ObjectList( U );
    nobsU := Length( obsU );
    post := Position( obsU, t );
    arayU := RayArrowsOfGroupoid( U );
    rt := arayU[ post ]^-1;
    arayt := List( arayU, r -> rt * r );
    gpt := ObjectGroup( U, t );
    rc := RightCoset( gpt, g );
    rcos := rec( groupoid := gpd, elements := rc, tobs := obsU, 
                 hobs := [ h ], rep := e, trays := arayt, type := "r" );
...
function( gpd, sgpd, e ) 
    ...
    local g, t, h, G, V, obsV, nobsV, posh, arayV, rh, arayh, gph, lc, lcos;
    ...
    if IsSinglePiece( sgpd ) then 
        V := sgpd;
        if not ( h in ObjectList( V ) ) then
            Error( "head of e not in V" );
        fi;
    else
        Info( InfoGroupoids, 2, "comment: sgpd is not a single piece!" );
        V := PieceOfObject( sgpd, h );
        if ( V = fail ) then 
            Error( "head of e is not an object in sgpd" );
        fi;
    fi;
    if not IsSubgroupoid( G, V ) then 
        Error( "V not a subgroupoid of G," );
    fi;
    obsV := ObjectList( V );
    nobsV := Length( obsV );
    posh := Position( obsV, h );
    arayV := RayArrowsOfGroupoid( V );
    rh := arayV[ posh ]^-1;
    arayh := List( arayV, r -> rh*r );
    gph := ObjectGroup( V, h );
    lc := LeftCoset( g, gph );
    lcos := rec( groupoid := gpd, elements := lc, tobs := [ t ],
                 hobs := obsV, rep := e, hrays := arayh, type := "l" );
...
function( gpd, lsgpd, rsgpd, e ) 
    ...
    local g, t, h, G, U, V, obsU, nobsU, arayU, post, rt, arayt, gpt, rc,
          obsV, nobsV, arayV, posh, rh, arayh, gph, lc, dc, dcos;
    ...
    gph := ObjectGroup( V, h );
    lc := LeftCoset( g, gph );
    dc := DoubleCoset( gpt, g, gph );
    dcos := rec( groupoid := gpd, elements := dc, tobs := obsU, hobs := obsV, 
                 rep := e, trays := arayt, hrays := arayh, type := "d" );
...
function( e, hc ) 
    ...
    if ( hc!.type = "h" ) then ## homset 
        ...
        pos := Position( hc!.hobs, e![4] );
        if ( pos = fail ) then return false; fi;
        r := e![2] * hc!.hrays[pos]![2]^-1;
        if not ( r in hc!.elements ) then return false; fi;
        return true;
    elif ( hc!.type = "d" ) then ## double coset   
        Print( "'in' not yet implemented for double cosets\n" );
        return false;
    else 
...
function( G, U )
    ...
    for P in Pieces( U ) do
        ...
        for o in obP do 
            ...
            for r in rco do
                Add( reps, ArrowNC( G, 1, Representative(r), o, o ) );
            od;
        od;
        ...
        for o in obs do 
            for r in rco do
                Add( reps, ArrowNC( G, 1, Representative(r), ro, o ) );
            od;
        od;
    od;
    return reps;
    ...
    for P in Pieces( U ) do
        ...
        for o in obP do 
            ...
            for r in rco do 
                ...
                Add( reps, ArrowNC( G, 1, rep^(-1), o, o ) );
            od;
        od;
        obs := Difference( obG, obP );
        ro := obP[1];
        ...
        for o in obs do 
            for r in rco do 
                rep := Representative( r );
                Add( reps, ArrowNC( G, 1, rep^(-1), o, ro ) );
            od;
        od;
    od;
    return reps;
...
function( e1, e2 )
    ...
    return ArrowNC( par, 1, e1![2]*e2![2], e1![3], e2![4] );
end );
...
function( e, n )
    ...
    if ( n = 1 ) then
        ...
        return ArrowNC( e![1], 1, e![2]^-1, h, t );
    elif ( t = h ) then
        return ArrowNC( e![1], 1, e![2]^n, t, h );
    fi;
    return fail;
end );
...
function( e1, e2 )
    ...
    if ( e2![4] = p ) then  
        if ( e1![3] = p ) then 
            if ( e1![4] = p ) then 
                return ArrowNC( gpd, 1, e1![2]^c, p, p );
            else 
                return ArrowNC( gpd, 1, c^(-1)*e1![2], p, e1![4] );
            fi;
        elif ( e1![4] = p ) then 
            return ArrowNC( gpd, 1, e2![2]*c, e1![3], p );
        else 
            return e1;
        fi;
    else 
        ...
        if ( e1![3] = p ) then 
            if ( e1![4] = p ) then
                return ArrowNC( gpd, 1, e1![2]^c, q, q );
            elif ( e1![4] = q ) then 
                return ArrowNC( gpd, 1, c^(-1)*e1![2]*c^(-1), q, p );
            else 
                return ArrowNC( gpd, 1, c^(-1)*e1![2], q, e1![4] );
            fi;
        elif ( e1![3] = q ) then 
            if ( e1![4] = q ) then 
                return ArrowNC( gpd, 1, e1![2]^(c^(-1)), p, p );
            elif ( e1![4] = p ) then 
                return ArrowNC( gpd, 1, c*e1![2]*c, p, q );
            else 
                return ArrowNC( gpd, 1, c*e1![2], p, e1![4] );
            fi;
        elif ( e1![4] = p ) then 
            return ArrowNC( gpd, 1, e1![2]*c, e1![3], q );
        elif ( e1![4] = q ) then 
            return ArrowNC( gpd, 1, e1![2]*c^(-1), e1![3], p );
        else 
            return e1;
        fi;
    fi;
...
function( gpd, obj ) 
    ...
    if ( HasIsGroupoidByIsomorphisms( gpd ) 
        ...
        return ArrowNC( gpd, 1, One(gpd!.magma), obj, obj );
    fi;
end );
...
function( gpd, obj )
    local comp;
    comp := PieceOfObject( gpd, obj );
    return ArrowNC( gpd, 1, One(comp!.magma), obj, obj );
end );
...
function( D, i )
    ...
    for j in [1..ngens] do 
        ...
        images[j] := ArrowNC( g![1], 1, pg, g![3][i], g![4][i] );
    od;
...
function( D, i )
    ...
    for j in [1..ngens] do 
        ...
        images[j] := ArrowNC( D, 1, eg, imobG[tg], imobG[hg] );
    od;
...
function( gp, obs, rays )
    ...
    if not IsHomogeneousList( [ One(gp), rays[1] ] ) then 
